...
import time
from openmeteo_attributes import hourly_attributes
...
class OpenMeteoAPI:
    ...
    def get_hourly_forecast_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for index, row in df.iterrows():
            ...
            df = self._get_hourly_forecast(latitude, longitude)
            result_df = pd.concat([result_df, df], ignore_index=True)

        return result_df

    def _get_hourly_forecast(self, latitude: float, longitude: float) -> pd.DataFrame:
